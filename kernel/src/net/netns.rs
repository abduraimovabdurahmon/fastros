...
pub fn host() -> Arc<NetNs> {
    HOST.call_once(|| Arc::new(NetNs { id: String::from("host"), kind: Kind::Host })).clone()
}
...
/// Create (or fetch) a private namespace keyed by `id` (a container id). Each
/// gets a fresh isolated loopback stack.
pub fn create(id: &str) -> Arc<NetNs> {
    let mut map = NAMESPACES.lock();
    if let Some(ns) = map.get(id) {
        return ns.clone();
    }
    let ns = Arc::new(NetNs { id: id.to_string(), kind: Kind::Private(Mutex::new(super::new_loopback_stack())) });
...
/// Drop a private namespace (on container removal). Any sockets still holding an
/// `Arc<NetNs>` keep it alive until they too are dropped.
